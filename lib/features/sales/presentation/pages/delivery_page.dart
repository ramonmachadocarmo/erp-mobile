...
class DeliveryPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final orders = (ref.watch(salesOrdersProvider).valueOrNull ?? [])
        .where((o) => o.status == 'PICKED' || o.status == 'DELIVERED' || o.status == 'UNDELIVERED')
        .toList();
    final people = {
      for (final c in ref.watch(customersProvider).valueOrNull ?? []) c.id: c,
    };
    final customers = {for (final e in people.entries) e.key: e.value.displayName};
                          ...
                          children: [
                            if (o.status == 'DELIVERED')
                              FilledButton(
                                ...
                                onPressed: () async {
                                  try {
                                    await ref.read(salesOrdersProvider.notifier).undoDeliver(o.id);
                                  } catch (e) {
                                ...
                                onPressed: () => Navigator.of(context).push(
                                  MaterialPageRoute<void>(
                                    builder: (_) => PickingLabelsPage(
                                      order: o,
                                      customerName: customers[o.customerId] ?? o.customerId,
                              ...
                              FilledButton(
                                onPressed: () async {
                                  try {
                                    await ref.read(salesOrdersProvider.notifier).deliver(o.id);
                                  } catch (e) {
                                    if (context.mounted) showError(context, '$e');
                                  }
                                },
                              ...
                              OutlinedButton(
                                onPressed: () async {
                                  ...
                                  try {
                                    await ref.read(salesOrdersProvider.notifier).failDelivery(o.id, note);
                                  } catch (e) {
                                    if (context.mounted) showError(context, '$e');
                                  }
                                },
